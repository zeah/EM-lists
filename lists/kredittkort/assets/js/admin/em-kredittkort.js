...
//	https://europe-west1-axo-data.cloudfunctions.net/function-1?affiliate=adservice&campaign=test&media=tast%20site&payout=1&clicktime=&status=approve&currency=SEK&info=source:bing|clid:test2
	...
	let newinput = (o = {}) => {
		...
		if (o.type != 'checkbox') {
			if (!o.sort) input.setAttribute('value', (kredittkort_data.meta[o.name] == undefined) ? '' : kredittkort_data.meta[o.name]);
			else {
				let sort = kredittkort_data.kredittkort_sort;

				if (o.sort != 'default') sort = kredittkort_data['kredittkort_sort_'+o.sort];

				if (sort == undefined) sort = kredittkort_data.kredittkort_sort;

				input.setAttribute('value', sort);
			}
		}
		else if (kredittkort_data.meta[o.name]) input.setAttribute('checked', '');
		else if (kredittkort_data[o.name]) input.setAttribute('checked', '');
	...
	let dicedropdown = (o = {}) => {
		...
		let addOption = (o = {}) => {
			...
			if (o.value == kredittkort_data.meta.terning) option.setAttribute('selected', '');
	...
	for (let sort of kredittkort_data['tax'])
		container_sort.appendChild(newinput({
	...
	info_container.appendChild(newinput({name: 'info05', title: 'Text 05 (maks kreditt)'}));
	info_container.appendChild(newinput({name: 'info02', title: 'Text 02'}));
	info_container.appendChild(newinput({name: 'info06', title: 'Text 06 (rentefrie dager)'}));
	info_container.appendChild(newinput({name: 'info03', title: 'Text 03'}));
	info_container.appendChild(newinput({name: 'info07', title: 'Text 07 (aldersgrense)'}));

	info_container.appendChild(newinput({name: 'info04', title: 'blurb'}));
	info_container.appendChild(newinput({name: 'info08', title: 'rente eksempel'}));
...
(() => {
	...
	// if (kredittkort_data && kredittkort_data['meta'])
	for (let sort of kredittkort_data['meta'])
		container_sort.appendChild(newinput({
			name: 'kredittkort_sort_'+sort, 
			title: 'Sortering '+sort.replace(/-/g, ' '), 
			notData: true, 
