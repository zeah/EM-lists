(() => {
	...
	console.log(emkredittkortse_data);
	...
	let newinput = (o = {}) => {
		...
		if (o.type != 'checkbox') {
			if (!o.sort) input.setAttribute('value', (emkredittkortse_data.meta[o.name] == undefined) ? '' : emkredittkortse_data.meta[o.name]);
			else {
				let sort = emkredittkortse_data.emkredittkortse_sort;

				if (o.sort != 'default') sort = emkredittkortse_data['emkredittkortse_sort_'+o.sort];

				if (sort == undefined) sort = emkredittkortse_data.emkredittkortse_sort;

				input.setAttribute('value', sort);
			}
		}
		else if (emkredittkortse_data.meta[o.name]) input.setAttribute('checked', '');
		else if (emkredittkortse_data[o.name]) input.setAttribute('checked', '');
	...
	let dicedropdown = (o = {}) => {
		...
		let addOption = (o = {}) => {
			...
			if (o.value == emkredittkortse_data.meta.terning) option.setAttribute('selected', '');
	...
	for (let sort of emkredittkortse_data['tax'])
		container_sort.appendChild(newinput({
	...
	// container.appendChild(newinput({name: 'emkredittkortse_redirect', type: 'checkbox', title: 'Add Redirection', notData: true}));
